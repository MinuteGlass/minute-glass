...
// Prix unitaire de référence = pack solo (1 jeton)
const UNIT_PRICE_CENTS = PACKS.find((p) => p.id === "solo")!.priceCents;
...
export default function TarifsPage() {
  ...
  const packForQty =
    PACKS.find((p) => p.tokens >= tokensNeeded) ?? PACKS[PACKS.length - 1];
  ...
  async function handleBuy(packId: string) {
    ...
    try {
      ...
      if (!token) { window.location.href = "/partenaire"; return; }
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F4F6F5" }}>
      ...
      <section className="max-w-[900px] mx-auto px-6 mb-14">
        <div className="bg-white rounded-2xl overflow-hidden" style={{ border: "1px solid #EAEFED", boxShadow: "0 1px 3px rgba(17,33,27,.04)" }}>
          <div className="px-6 py-4" style={{ borderBottom: "1px solid #EAEFED" }}>
            ...
            <p className="m-0 mt-1 text-[13px] font-semibold" style={{ color: "#6B7280" }}>
              1 jeton = {formatPrice(UNIT_PRICE_CENTS)} € · prix unitaire de référence
            </p>
          </div>
          <div className="divide-y" style={{ borderColor: "#EAEFED" }}>
            {[
              { label: "Réparation impact",       sub: "assuré ou non assuré",     tokens: 1, color: "#1D9E75", bg: "#E8F6F0" },
              { label: "Vitre latérale",           sub: "assuré ou non assuré",     tokens: 1, color: "#6D28D9", bg: "#EDE9FE" },
              { label: "Remplacement pare-brise",  sub: "sans assurance BdG",       tokens: 2, color: "#B0431F", bg: "#FCEDE7" },
              { label: "Remplacement pare-brise",  sub: "avec assurance BdG",       tokens: 3, color: "#0F5C44", bg: "#E8F6F0" },
            ].map((row) => (
              <div key={row.label + row.sub} className="flex items-center justify-between px-6 py-4 gap-4">
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                  ...
                  <span className="text-[14px] font-extrabold w-16 text-right" style={{ color: "#11211B" }}>
                    {formatPrice(row.tokens * UNIT_PRICE_CENTS)} €
                  </span>
                </div>
              </div>
            ))}
      ...
      <section className="max-w-[900px] mx-auto px-6 mb-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
          {PACKS.map((p) => (
            <div
              key={p.id}
              className="rounded-[20px] p-6 flex flex-col gap-3 relative"
              style={p.highlight
            ...
            >
              ...
              <div className={`font-extrabold text-[15px] ${p.highlight ? "opacity-90" : ""}`} style={!p.highlight ? { color: "#6B7280" } : {}}>
                {p.badge} {p.name}
              </div>
              <div className="flex items-end gap-2 leading-none">
                <span className="text-[40px] font-extrabold tracking-tight">{p.tokens}</span>
                <span className={`text-[15px] font-bold mb-1.5 ${p.highlight ? "opacity-80" : ""}`} style={!p.highlight ? { color: "#6B7280" } : {}}>
                  jeton{p.tokens > 1 ? "s" : ""}
                </span>
              </div>
              <div className="flex items-end gap-1">
                <span className="text-[30px] font-extrabold tracking-tight" style={!p.highlight ? { color: "#0F5C44" } : {}}>
                  {formatPrice(p.priceCents)} €
                </span>
              </div>
              <div className={`text-[12px] font-semibold ${p.highlight ? "opacity-80" : ""}`} style={!p.highlight ? { color: "#6B7280" } : {}}>
                {pricePerToken(p)} €/jeton · sans expiration
              </div>
              <button
                onClick={() => handleBuy(p.id)}
                disabled={loadingPack === p.id}
                className="w-full py-3 rounded-[11px] font-bold text-[13px] border-0 cursor-pointer transition-opacity hover:opacity-90 mt-2 disabled:opacity-60"
                style={p.highlight
                  ? { background: "#fff", color: "#0F5C44" }
                  : { background: "#1D9E75", color: "#fff", boxShadow: "0 4px 12px rgba(29,158,117,.25)" }
                }
              >
                {loadingPack === p.id ? "…" : "Acheter"}
              </button>
            </div>
          ))}
        </div>
          ...
          <div className="max-w-[480px] mx-auto flex flex-col gap-5">
            ...
            <div>
              <label className="block text-[13px] font-bold mb-2">
                Nombre de fiches / mois : <span style={{ color: "#0F5C44" }}>{simQty}</span>
              </label>
              <input
            ...
            <div className="rounded-[14px] p-5 text-center" style={{ background: "#F4F6F5", border: "1px solid #EAEFED" }}>
              ...
              <div className="text-[22px] font-extrabold mt-1" style={{ color: "#0F5C44" }}>
                Pack {packForQty.name} — {formatPrice(packForQty.priceCents)} €
              </div>
              {surplus > 0 && (
      ...
      <section className="text-center py-14 px-6 mb-10 mx-6 rounded-[20px] max-w-[900px] mx-auto" style={{ background: "linear-gradient(150deg,#0F5C44,#1D9E75)" }}>
        ...
        <p className="m-0 mt-3 text-[15px] font-medium text-white opacity-90">Créez votre compte réparateur gratuitement et commencez à débloquer des fiches dès aujourd'hui.</p>
