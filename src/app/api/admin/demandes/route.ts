...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    .from("demandes")
    .select("id, title, city, intervention, insurance, damage, availability, status, verified, prix_dossier, margin_prix, margin_franchise, margin_pieces, created_at, client_id")
...
export async function PATCH(req: NextRequest) {
  ...
  const { id, verified, prix_dossier, margin_prix, margin_franchise, margin_pieces } = await req.json();
  if (!id) return NextResponse.json({ error: "id manquant" }, { status: 400 });

  const toNum = (v: unknown) => v === undefined ? undefined : (v === "" || v === null) ? null : Number(v);
  const updates: Record<string, unknown> = {};
  if (verified !== undefined) updates.verified = verified;
  if (prix_dossier !== undefined) updates.prix_dossier = toNum(prix_dossier);
